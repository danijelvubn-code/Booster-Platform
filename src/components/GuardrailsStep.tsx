		...
		<div className="flex items-center justify-between gap-4">
			...
			<div className="flex items-center gap-4">
				...
				<Select
					value={config.action}
					onValueChange={(v) =>
						onChange({ ...config, action: v as GuardrailAction })
					}
...
const GuardrailsStep = ({ state, onChange }: GuardrailsStepProps) => {
	const update = <K extends keyof GuardrailsState>(
		key: K,
		value: GuardrailsState[K],
	) => {
		...
		<div className="space-y-1">
			<div className="mb-4">
				...
				<p className="text-sm text-muted-foreground mt-1">
					Configure safety, content filtering, adversarial prompt protection,
					and privacy controls for this endpoint.
				</p>
			</div>
			...
			<div className="rounded-md border border-border bg-muted/50 p-3 flex items-start gap-2 mt-4">
				...
				<p className="text-xs text-muted-foreground">
					Guardrails may slightly impact latency depending on selected
					performance profile.
				</p>
			</div>
		</div>
...
export const GuardrailsReviewSection = ({
	state,
}: {
	state: GuardrailsState
}) => {
