          ...
          <ArrowLeft className="h-4 w-4 mr-1" /> Model Cosmos
            ...
            <PopoverContent className="w-72 p-0" align="end">
              <div className="p-3 border-b border-border">
                <p className="text-xs font-semibold text-muted-foreground">Load a use-case profile</p>
              </div>
              <div className="max-h-56 overflow-y-auto p-1">
                {savedProfiles.length === 0 ? (
                  <p className="text-xs text-muted-foreground text-center py-4">
                    No saved profiles yet. Use "Save" to create one.
                  </p>
                ) : (
                  <>
                    {/* Platform-provided templates */}
                    {savedProfiles.filter((p) => p.platform).length > 0 && (
                      <div className="px-2 pt-2 pb-1">
                        <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Platform Templates</p>
                      </div>
                    )}
                    {savedProfiles.filter((p) => p.platform).map((p) => (
                      <div
                        key={p.id}
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                            <span>{p.state.useCase}</span>
                            <span>·</span>
                            <span>{p.state.objective}</span>
                          </div>
                        </div>
                        <Badge variant="outline" className="text-[9px] px-1.5 shrink-0">Template</Badge>
                      </div>
                    ))}
                    ...
                    {savedProfiles.filter((p) => !p.platform).length > 0 && (
                      <div className="px-2 pt-3 pb-1">
                        <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Your Profiles</p>
                      </div>
                    )}
                    {savedProfiles.filter((p) => !p.platform).map((p) => (
                      <div
                        key={p.id}
                        className={cn(
                          "flex items-center gap-2 px-2 py-2 rounded-sm hover:bg-accent cursor-pointer",
                          loadedProfileId === p.id && "bg-accent"
                        )}
                        onClick={() => loadProfile(p)}
                      >
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate">{p.name}</p>
                          <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                            <span>{p.state.useCase || "No use case"}</span>
                            <span>·</span>
                            <span className="flex items-center gap-0.5">
                              <Clock className="h-2.5 w-2.5" />
                              Step {p.lastStep + 1}/{stepsMeta.length}
                            </span>
                          </div>
                        </div>
                        <div className="flex items-center gap-1 shrink-0">
                          {loadedProfileId === p.id && (
                            <Badge variant="secondary" className="text-[9px] px-1">Active</Badge>
                          )}
                          <Button
                            variant="ghost"
                            size="icon"
      ...
      {loadedProfileId && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-primary/5 border border-primary/20">
          ...
          <span className="text-xs text-muted-foreground">
            Editing: <span className="font-medium text-foreground">{savedProfiles.find(p => p.id === loadedProfileId)?.name}</span>
          </span>
          <Button
            variant="ghost"
            size="icon"
            className="h-5 w-5 ml-auto"
            onClick={() => { setLoadedProfileId(null); setState(defaultState); setStep(0); }}
          >
            <X className="h-3 w-3" />
          </Button>
        </div>
