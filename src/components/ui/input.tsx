...
>(({ className, invalid, success, fieldDisabled, fieldReadOnly, size = "md", ...props }, ref) => (
  <div
    ref={ref}
...
const InputPrefixAddon = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
...
const InputSegment = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn("flex min-h-0 min-w-0 flex-1 items-center gap-2 self-stretch px-3", className)} {...props} />
));
InputSegment.displayName = "InputSegment";

const InputLeadIcon = ({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) => (
  <span className={cn("inline-flex shrink-0 text-foreground/50 [&_svg]:h-icon-16 [&_svg]:w-icon-16", className)} {...props} />
);
InputLeadIcon.displayName = "InputLeadIcon";

const InputTrailIcon = ({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) => (
  <span className={cn("inline-flex shrink-0 text-foreground/50 [&_svg]:h-icon-16 [&_svg]:w-icon-16", className)} {...props} />
);
InputTrailIcon.displayName = "InputTrailIcon";

const InputLoadingIndicator = ({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) => (
  <span className={cn("inline-flex shrink-0 text-foreground/50", className)} {...props}>
...
const InputControl = React.forwardRef<HTMLInputElement, React.ComponentProps<"input">>(({ className, disabled, readOnly, ...props }, ref) => (
  <input
    ref={ref}
    disabled={disabled}
    readOnly={readOnly}
...
type InputClearButtonProps = Omit<React.ComponentProps<typeof Button>, "children"> & {
  "aria-label": string;
};

const InputClearButton = React.forwardRef<HTMLButtonElement, InputClearButtonProps>(({ className, ...props }, ref) => (
  <Button ref={ref} type="button" variant="ghost" size="icon-sm" className={cn("-mr-1 shrink-0", className)} {...props}>
...
const InputSuffixAddon = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
...
function InputAddonText({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) {
  return <span className={cn("whitespace-nowrap text-body-sm text-foreground/75", className)} {...props} />;
}
...
const InputField = React.forwardRef<HTMLInputElement, InputFieldProps>(
  (
    ...
    const leftAddon = prefixAddon ?? (prefixValue ? <InputAddonText>{prefixValue}</InputAddonText> : null);
    const rightAddon =
      suffixAddon ??
      ...
      <div className={cn("w-full space-y-1", className)}>
        {label !== undefined && (
          <div className="flex items-center justify-between">
            <Label htmlFor={id} className={cn("inline-flex items-center gap-1", disabled && "text-foreground/50")}>
              {label}
              {required && (
                <>
        ...
        >
          ...
          <InputSegment>
            ...
            {showClear && <InputClearButton aria-label={clearLabel} onClick={handleClear} />}
