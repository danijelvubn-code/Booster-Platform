 ...
 * **Product MVP vs post-MVP UX is route-based** (`/flows/mvp`, `/mvp/*` vs `/flows/post-mvp`, `/mvp/post-mvp/*`, …).
 * Use `npm run dev:mvp` / `npm run dev:post-mvp` only if you read `getAppVariant()` elsewhere; it does not switch login pages.
 */
