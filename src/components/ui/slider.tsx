...
const sliderRangeVariants = cva("absolute bg-primary group-data-[disabled]:bg-primary/50", {
  variants: {
    orientation: {
      horizontal: "h-full",
      vertical: "w-full",
...
function markIsOnFilledRange(m: number, min: number, sliderValues: number[]): boolean {
...
const Slider = React.forwardRef<React.ElementRef<typeof SliderPrimitive.Root>, SliderProps>(
  (
    ...
    const [liveValue, setLiveValue] = React.useState<number[] | undefined>(undefined);
    ...
    return (
      ...
      >
        <SliderPrimitive.Track className={sliderTrackVariants({ orientation, size })}>
          <SliderPrimitive.Range className={cn("z-0", sliderRangeVariants({ orientation }))} />
          {showMarksInTrack ? (
            <SliderStepMarksInTrack min={min} max={max} values={markValues} sliderValues={sliderValuesForMarks} />
          ) : null}
        </SliderPrimitive.Track>
        {Array.from({ length: thumbCount }).map((_, index) => (
          <SliderPrimitive.Thumb key={index} className={sliderThumbVariants({ size })} />
        ))}
      </SliderPrimitive.Root>
    );
  },
