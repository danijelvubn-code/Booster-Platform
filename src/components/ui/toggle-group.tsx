...
import { toggleVariants, type ToggleSize } from "@/components/ui/toggle";
...
const toggleGroupRootVariants = cva("inline-flex w-fit max-w-full shrink-0", {
  ...
  compoundVariants: [
    { layout: "segmented", size: "sm", class: "h-control-sm items-stretch rounded-md p-0.5" },
    { layout: "segmented", size: "md", class: "h-control-md items-stretch rounded-md p-0.5" },
    { layout: "segmented", size: "lg", class: "h-control-lg items-center rounded-lg px-1 py-1" },
  ],
  defaultVariants: {
    layout: "segmented",
    size: "md",
...
const ToggleGroup = React.forwardRef<React.ElementRef<typeof ToggleGroupPrimitive.Root>, ToggleGroupProps>(
  ({ className, variant = "default", size = "md", layout = "segmented", children, ...props }, ref) => (
    <ToggleGroupPrimitive.Root
      ref={ref}
      className={cn(toggleGroupRootVariants({ layout, size }), className)}
      {...props}
    >
      <ToggleGroupContext.Provider value={{ variant, size, layout }}>{children}</ToggleGroupContext.Provider>
    </ToggleGroupPrimitive.Root>
  ),
);
  ...
  return (
    <ToggleGroupPrimitive.Item
      ...
      className={cn(
        ...
        layout === "segmented" && (s === "sm" || s === "md") && "h-full min-h-0 rounded-sm",
        layout === "segmented" && s === "lg" && "h-control-md min-h-0 rounded-md",
        segmentedChrome,
        className,
      )}
      {...props}
