	...
	return (
		...
		>
			...
			<span className="min-w-0 flex-1 text-left">
				<span className="block font-semibold text-body-sm leading-normal text-foreground">
					{title}
				</span>
				<span className="block text-body-sm leading-normal text-muted-foreground">
					{description}
				</span>
			</span>
			<span
			...
			<DialogContent className="flex max-h-screen max-w-modal-lg flex-col gap-0 overflow-hidden bg-background p-0 shadow-lg sm:rounded-xl [&>button]:hidden">
				<div className="flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto p-8 pb-6">
					...
					<p className="text-body-sm leading-relaxed text-foreground">
						Browse available models in Model Cosmos — your model library. Each
						Inference Endpoint has one stable URL. The Default model serves all
						live traffic. Your input data and model responses are never used to
						train our models and will not be shared with other users or third
						parties.
					</p>
					...
					<div className="flex gap-3 rounded-lg bg-muted px-4 py-3 text-foreground">
						...
						<p className="min-w-0 flex-1 text-body-sm leading-relaxed text-foreground">
							Your input data and model responses are never used to train our
							models and will not be shared with other users or third parties.
						</p>
						<BadgeCheck
